...
def load_data(tokenizer, seed=69, cache_dir="cache", num_examples=1000, batch_size=16):
    ...
    def process_batch(batch):
        ...
        attention_mask_list = []
        labels_list = []
        for prompt, output_label in zip(batch["reannotated_messages"], batch["reannotated_assistant_content"]):
            # Remove the last message (assistant's response)
            prompt = prompt[:-1]
            # Apply chat template
            ...
            # Tokenize
            tokenized_input = tokenizer(text, return_tensors="pt", truncation=True, padding="longest", max_length=4096)
    ...
    raw_dataset = load_dataset("ServiceNow-AI/R1-Distill-SFT", "v1", split='train', cache_dir=cache_dir)
